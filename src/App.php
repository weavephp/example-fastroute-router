...
class App
{
	 ...
	 *
	 * In the example here, we are using FastRoute - note that what happens in this method
	 * very much depends on which Router you decide to use. In the case of FastRoute,
	 * a \FastRoute\RouteCollector instance is provided as $router.
	 *
	 * Here we are setting up a single route that simply routes '/' with an optional
	 * value which is provided as a Request attribute called 'owner'. If you called
	 * this app with '/wibble' then 'owner' would be set to 'wibble'. Unlike in the
	 * example-doublepass example app which uses Aura.Router, FastRoute doesn't define
	 * default values so the default is handled in UppercaseOwner to keep the example
	 * as close as possible to the others. In reality, you'd prob. not want to mix
	 * behaviours by having a single middleware both provide defaults and perform an action.
	 *
